package adriver{
	...
	public class adriverLoader extends Sprite {
		private const VERSION:String = "1.98";
		...
		private function get_right_custom(custom:Object):String {
			...
			for ( var i:int=0; i < custom.length; i++) {
				if (custom[i]) {
					s.push( (!j?(j=1,i+'='):'')+encode(custom[i]));
				}
				else {
					j=0;
				}
		...
		// urlencode compatible encoding
		// lifted from http://code.google.com/p/oauth-as3/
		 ...
		 */
		private static function urlEncode(string:String):String {
